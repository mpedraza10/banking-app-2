...
export function PaymentDenominationInterface({
  ...
  const totalsMatch = Math.abs(calculatedTotal - paymentAmount) < 0.01;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Error al registrar denominaciones de pago"
      );
    } finally {
    ...
    <div className="space-y-6">
      <Alert className="bg-blue-50 border-blue-200">
        <AlertDescription className="text-blue-800">
          Ingresa el desglose de denominaciones para el pago de ${paymentAmount.toFixed(2)}
        </AlertDescription>
      </Alert>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {bills.map((denomination) => {
              ...
              const inputId = `bill-${denomination}`;
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {coins.map((denomination) => {
              const qty = denominations[denomination] || 0;
              const count = qty > 0 ? `(${qty})` : "";
              const inputId = `coin-${denomination}`;
